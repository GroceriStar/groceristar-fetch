...
describe('testing if methods inside the files are accessible from the outside', () => {
  ...
  it('[ChickenKyiv] get recipe by title file is set', () => {
    var stream = chickenKyiv.getRecipeByTitle('Lemonade')
    expect(stream).not.toBe('')
  })

  it('[ChickenKyiv] fetch random recipe file is set', () => {
...
describe('testing if methods inside the files are accessible from the outside', () => {
  ...
  it('[GS-Loopback] fetch Ultimate Grocery List', () => {
    var stream = loopback.getUltimateGrocery()
    expect(stream).not.toBe('')
  })

  it('[GS-Loopback] fetch grocery with user relations', () => {
    var stream = loopback.getGLwithUserRelations()
    expect(stream).not.toBe('')
  })

  it('[GS-Loopback] get loopback sample data', () => {
    var stream = loopback.getIngredientsSampleFromDB()
    expect(stream).not.toBe('')
  })

  it('[GS-Loopback] fetch a useronject data with a name argument in loopback ', () => {
...
describe('testing if methods inside the files are accessible from the outside', () => {
  ...
  it('[GraphQL] fetch a list keys of user grocery', () => {
...
describe('Groceristar methods check', () => {
  ...
  it('[Groceristar] departments file is set', () => {
    var stream = groceristar.getDepartments()
    expect(stream).not.toBe('')
  })

  it('[Groceristar] showcase checking', () => {
...
describe('Search Form methods check', () => {
  // @TODO format it later
  it('[Search Form] getAttribute("allergy")', () => {
